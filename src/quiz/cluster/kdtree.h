...
struct KdTree
{
	...
	void searchHelper(std::vector<float> target, Node* node, uint depth, float distanceTol, std::vector<int> &ids)
	{
		if (node != NULL)
		{
			...
			uint cd = depth % 2;
			if ( (target[cd] - distanceTol) < node->point[cd] )		// part of the box is on the left of the split line
				searchHelper(target, node->left, depth+1, distanceTol, ids);

			if ( (target[cd] + distanceTol) > node->point[cd] )		// part of the box is on the right of the split line
